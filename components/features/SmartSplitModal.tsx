...
export default function SmartSplitModal({
  ...
  const fetchPreview = async (investAmount: string) => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await fetch(
        "http://localhost:4000/api/v1/allocation/preview",
        {
          method: "POST",
          headers: {
  ...
  const executeSmartSplit = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await fetch(
        "http://localhost:4000/api/v1/allocation/execute",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        <div className="bg-surface rounded-2xl p-8 max-w-md w-full">
          <div className="text-center mb-6">
            ...
            <h2 className="text-2xl font-bold text-text-primary mb-2">
              Set Up Smart Split First
            </h2>
            <p className="text-text-secondary">
              You need to configure your allocation preferences before using
              Smart Split.
            </p>
          </div>
